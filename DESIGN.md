...
| 2026-06-01 | **Mesh layout pivot: d3-force → dagre layered genealogy layout** | The force-directed mesh read as a *tangle*, not a family tree (no generational structure, couples ungrouped, crossing edges, positions re-scrambling on every add). Pivoted the V0 default to a **layered layout via dagre** (React Flow's recommended practice; the right tool for a lineage/DAG). Each couple is a thin **union-junction node**: both partners → knot → children centered below; single-parent children get a direct edge. dagre is deterministic, so adding a person *nudges* the tree instead of re-scrambling it. **Nuances the 2026-05-25 "force-directed for the most-alive feel" decision:** aliveness now comes from the fieldbook styling + motion + drying-ink, not chaotic positioning. (d3-force/d3-dag remain candidates for an optional "organic"/"tidy" toggle later.) |
| 2026-06-01 | Alive motion + focused-card relationships | Cards ease in on first load + when added; existing cards glide to new positions on layout shifts (gated so they don't fly in from the origin); all `prefers-reduced-motion`-gated. The focused detail card now reads as "the page about this person": name + dates + fresh tag, then relationships in the marginalia voice (*hijo/a de … · en pareja con … · madre/padre de …*), then edit. |
